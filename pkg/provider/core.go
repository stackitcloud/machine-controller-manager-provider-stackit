...
func (p *Provider) InitializeMachine(_ context.Context, req *driver.InitializeMachineRequest) (*driver.InitializeMachineResponse, error) {
	klog.V(2).Infof("InitializeMachine request has been received for %q", req.Machine.Name)
	defer klog.V(2).Infof("InitializeMachine request has been processed successfully for %q", req.Machine.Name)
