...
func (p *Provider) CreateMachine(ctx context.Context, req *driver.CreateMachineRequest) (*driver.CreateMachineResponse, error) {
	...
	projectID, serviceAccountKey := decodeCloudProviderSecret(req.Secret)
	...
	if server == nil {
		...
		server, err = p.client.CreateServer(ctx, projectID, providerSpec.Region, p.createMachineRequest(req, providerSpec))
		if err != nil {
...
func (p *Provider) createMachineRequest(req *driver.CreateMachineRequest, providerSpec *api.ProviderSpec) *CreateServerRequest {
	...
	if providerSpec.Networking != nil {
		createReq.Networking.NetworkID = providerSpec.Networking.NetworkID
		createReq.Networking.NICIDs = providerSpec.Networking.NICIDs
	}
	...
	if providerSpec.UserData != "" {
		createReq.UserData = base64.StdEncoding.EncodeToString([]byte(providerSpec.UserData))
	}
